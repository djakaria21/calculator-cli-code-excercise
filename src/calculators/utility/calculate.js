	...
	for (const i of array_to_be_processed) {
		...
		//since we've checked that the equation is valid, we can be sure that we have enough numbers in our number stack to do each respective operation
			...
			if (i == "+") {
				...
				// reasoning behind using operation classes is we need to convert from the string to a number format like float.  It also will help if we have some new type of operation like sin or something
				value = add(y, x);
